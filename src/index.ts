...
export const Config = Schema.intersect([
  Schema.object({
    ...
    useHuaCache: Schema.boolean()
      ...
      .role('switch'),
    maxConcurrentThreads: Schema.number()
      .default(2)
      .min(1)
      .max(5)
      .step(1)
  ...
  Schema.object({
    ...
    showTranslationResult: Schema.boolean()
      ...
      .role('switch'),
    maxDrawCount: Schema.number()
      .default(1)
      .min(1)
      .max(10)
      .step(1)
      .description('单次重画命令最大生成数量(影响"重画N张"/"再来N张"命令)'),
    jrandomMin: Schema.number()
      .default(2)
      .min(1)
      .max(5)
      .step(1)
      .description('"几"的随机范围最小值'),
    jrandomMax: Schema.number()
      .default(4)
      .min(2)
      .max(10)
      .step(1)
  ...
  Schema.object({
    ...
    whitelistUsers: Schema.array(String)
      ...
      .role('table'),
    groupDailyLimits: Schema.array(Schema.object({
...
export function apply(ctx: Context) {
  ...
  ctx.middleware(async (session, next) => {
    ...
    if (!canGroupUse(groupId)) {
      ...
      if (!ctx.config.showRestrictionMessage) return next();
    }
  ...
  // 检查用户是否可以使用绘图功能
  function canUserDraw(userId: string, groupId?: string): boolean {
    // 如果未启用每日限制，直接返回 true
  ...
  // 获取用户剩余次数
  function getUserRemainingDraws(userId: string, groupId?: string): number {
    // 如果未启用每日限制，返回无限
    if (!ctx.config.enableDailyLimit) {
      return Infinity
    }
  ...
  // 获取群组特定的每日限制
  function getGroupDailyLimit(groupId?: string): number {
    // 如果没有提供群组ID，返回默认限制
    if (!groupId) {
  ...
  // 获取用户上一次提示词的函数
  function getUserLastPrompt(userId: string) {
  ...
  // 添加解析-O选项的函数
  function parseDisableArtistPrompt(text: string): { disableArtistPrompt: boolean, processedText: string } {
  ...
  async function generateImage(item: QueueItem, apiKey: string) {
    ...
    const noiseSchedule = getNoiseScheduleForModel(model, ctx.config.noiseSchedule)
      ...
      const response = await ctx.http.post('https://apis.loliy.top/v1/images/generate', requestData, {
        ...
        msg = await session.send(session.messageId ? h.quote(session.messageId) + content : content)
        }
  ...
  cmd.subcommand('.再来一张', '使用上次的提示词重新生成图片')
    ...
    .action(async ({ session }, ...args) => {
      try {
        ...
        // 处理中文数字和阿拉伯数字
        if (argsText) {
          if (argsText === '一' || argsText === '1') count = 1
          else if (argsText === '二' || argsText === '两' || argsText === '俩' || argsText === '2') count = 2
          else if (argsText === '三' || argsText === '3') count = 3
          else if (argsText === '四' || argsText === '4') count = 4
          else if (argsText === '五' || argsText === '5') count = 5
          else if (argsText === '六' || argsText === '6') count = 6
          else if (argsText === '七' || argsText === '7') count = 7
          else if (argsText === '八' || argsText === '8') count = 8
          else if (argsText === '九' || argsText === '9') count = 9
          else if (argsText === '十' || argsText === '10') count = 10
          else if (argsText === '几') {
            // 使用配置中设置的"几"的随机范围
        ...
        if (maxPosition > 1) {
          const queueMsg = await session.send(
            `正在使用上次的提示词生成 ${count} 张图片，队列位置: ${queuePositions.join(', ')}。\n前面还有 ${maxPosition - 1} 个请求在等待处理。${remainingMsg}`
          )
